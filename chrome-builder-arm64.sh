...
_create_chrome_appimage(){
	if wget --version | head -1 | grep -q ' 1.'; then
		wget -q --no-verbose --show-progress --progress=bar https://dl.google.com/linux/direct/"$APP"-"$CHANNEL"_current_arm64.deb
	else
		wget https://dl.google.com/linux/direct/"$APP"-"$CHANNEL"_current_arm64.deb
	fi
	ar x ./*.deb
	tar xf ./data.tar.xz
	mkdir "$APP".AppDir
	...
	-u "gh-releases-zsync|$GITHUB_REPOSITORY_OWNER|Chrome-appimage|continuous|*-$CHANNEL-*aarch64.AppImage.zsync" \
	./"$APP".AppDir Google-Chrome-"$CHANNEL"-"$VERSION"-aarch64.AppImage || exit 1
}
